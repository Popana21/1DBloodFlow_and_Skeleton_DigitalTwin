...
namespace Sample {
public class SkeletonSkullScript : MonoBehaviour
{
  ...
  private void STATUS ()
  {
      ...
      if(_Animator.GetCurrentAnimatorStateInfo(0).tagHash == AttackTag)
      {
          _Status[Attack] = true;
      }
      else if(_Animator.GetCurrentAnimatorStateInfo(0).tagHash != AttackTag)
      {
  ...
  private void CAMERA ()
  {
    _View_Camera.transform.position = this.transform.position + new Vector3(0, 1.0f, 3.0f);
  }
  //--------------------------------------------------------------------- GRAVITY
  ...
  private void FAINT ()
  {
    if (Input.GetKeyDown(KeyCode.W) && !_Status[Faint])
    {
